...
class DesignSystemGenerator:
    ...
    def _tokenize(self, text: str) -> set:
        """Tokenize text into lowercase alphanumeric words for loose scoring."""
        if not text:
            return set()
        return {
            token
    ...
    def _pick_value(self, row: dict, *keys: str, default: str = "") -> str:
        """Return the first non-empty field value from a row."""
        for key in keys:
            value = str((row or {}).get(key, "")).strip()
            if value:
                return value
        return default

    def _extract_google_stylesheet_url(self, typography_row: dict) -> str:
        """Prefer a real fonts.googleapis.com stylesheet URL over share links."""
        css_import = self._pick_value(typography_row, "CSS Import", "css_import")
        match = re.search(r"https://fonts\.googleapis\.com/[^'\"\s)]+", css_import)
        if match:
            return match.group(0)

        google_url = self._pick_value(
            typography_row,
            "Google Fonts URL",
            "google_fonts_url",
        )
    ...
    def _category_cluster(self, text: str) -> str:
        """Group queries into broad product families for landing selection."""
        tokens = self._tokenize(text)
        token_text = " ".join(sorted(tokens))

        clusters = {
            "editorial": {"blog", "news", "media", "magazine", "editorial", "content", "newsletter", "publishing", "article"},
            "portfolio": {"portfolio", "photographer", "photography", "designer", "creative", "studio", "artist", "agency", "personal"},
            "commerce": {"ecommerce", "commerce", "store", "shop", "catalog", "product", "marketplace", "retail", "inventory"},
            "dashboard": {"dashboard", "analytics", "admin", "metric", "metrics", "saas", "data", "ops", "operations", "monitoring"},
            "service": {"spa", "wellness", "beauty", "salon", "clinic", "restaurant", "hotel", "booking", "appointment", "service"},
        }
        ...
        if "media" in token_text:
            return "editorial"
        return ""

    def _is_mobile_app_intent(self, query: str, category: str) -> bool:
        """Detect explicit mobile app intent so app-store patterns stay opt-in."""
        haystack = self._tokenize(f"{query} {category}")
        strong_terms = {
            "ios", "android", "native", "playstore", "appstore",
            "download", "install", "reactnative", "expo",
        }
        site_terms = {
            "website", "site", "landing", "blog", "news", "portfolio",
            "store", "shop", "catalog", "spa", "booking", "restaurant",
            "agency", "dashboard", "admin",
        }

        if haystack & site_terms:
            return False
        if "app" in haystack and (haystack & strong_terms or "mobile" in haystack):
            return True
        return bool(haystack & strong_terms)

    def _select_color_match(self, results: list, category: str, query: str) -> dict:
        """Prefer category-aligned palettes instead of taking the first color row."""
        if not results:
            return {}
        ...
        for result in results:
            product = str(result.get("Product Type", "")).strip()
            product_lower = product.lower()
            product_tokens = self._tokenize(product)
            score = 0

            if product_lower == category_lower:
                score += 100
            elif product_lower and (product_lower in category_lower or category_lower in product_lower):
                score += 50

            score += 10 * len(product_tokens & category_tokens)
            score += 3 * len(product_tokens & query_tokens)
            scored.append((score, result))

        scored.sort(key=lambda item: item[0], reverse=True)
        return scored[0][1]

    def _select_landing_match(self, results: list, query: str, category: str, reasoning_pattern: str) -> dict:
        """Choose a landing pattern that fits the product instead of blindly taking rank 1."""
        if not results:
            return {}

        category_tokens = self._tokenize(category)
        query_tokens = self._tokenize(query)
        reasoning_tokens = self._tokenize(reasoning_pattern)
        cluster = self._category_cluster(f"{category} {query}")
        is_mobile_app = self._is_mobile_app_intent(query, category)
        trust_or_health = bool(query_tokens & {"pharmacy", "drug", "medical", "medicine", "health"} or category_tokens & {"pharmacy", "drug", "medical", "health"})

        cluster_preferences = {
            "editorial": {"newsletter", "content", "documentation", "faq", "community", "categories", "archives", "search", "editorial"},
            "portfolio": {"portfolio", "grid", "gallery", "storytelling", "showcase", "masonry", "case", "study"},
            "commerce": {"marketplace", "directory", "catalog", "product", "listing", "collection", "categories", "trust"},
            "dashboard": {"operations", "real", "time", "dashboard", "metrics", "analytics", "comparison", "pricing"},
            "service": {"storytelling", "booking", "appointment", "testimonial", "contact", "benefits", "service", "video"},
        }
        app_only_terms = {"app", "store", "download", "device", "screenshots", "install", "play"}
        cluster_penalties = {
            "editorial": {"event", "conference", "countdown", "waitlist", "configurator"},
            "portfolio": {"waitlist", "countdown", "comparison", "pricing", "conference", "event"},
            "commerce": {"event", "conference", "faq", "documentation", "newsletter", "waitlist", "countdown"},
            "dashboard": {"event", "conference", "portfolio", "gallery", "newsletter"},
            "service": {"event", "conference", "countdown", "waitlist", "documentation", "faq"},
        }
        opt_in_terms = {"waitlist", "countdown", "coming", "conference", "event", "pricing", "comparison", "review", "video", "faq"}

        scored = []
        for result in results:
            pattern_name = str(result.get("Pattern Name", ""))
            sections = str(result.get("Section Order", ""))
            result_text = f"{pattern_name} {sections} {result.get('Conversion Optimization', '')}"
            result_tokens = self._tokenize(result_text)
            score = 0

            score += 8 * len(result_tokens & reasoning_tokens)
            score += 6 * len(result_tokens & category_tokens)
            score += 2 * len(result_tokens & query_tokens)

            if cluster:
                score += 10 * len(result_tokens & cluster_preferences.get(cluster, set()))
                blocked = result_tokens & cluster_penalties.get(cluster, set())
                if blocked and not (query_tokens & opt_in_terms):
                    score -= 20 * len(blocked)

            if not is_mobile_app and result_tokens & app_only_terms:
                score -= 30
            if is_mobile_app and ("app" in result_tokens or "download" in result_tokens):
                score += 10
            if {"configurator", "customize", "customizer"} & result_tokens and not (query_tokens & {"3d", "configurator", "customize", "customizer"}):
                score -= 18
            if {"waitlist", "countdown"} & result_tokens and not (query_tokens & {"waitlist", "countdown", "coming", "launch", "preorder"}):
                score -= 24

            if cluster == "service" and {"booking", "appointment"} & query_tokens:
                if {"booking", "contact", "benefits", "video", "storytelling"} & result_tokens:
                    score += 12
            if cluster == "commerce":
                if {"catalog", "product", "purchase", "listing", "collection", "review", "buy"} & result_tokens:
                    score += 10
                if trust_or_health and {"review", "rating", "trust", "product"} & result_tokens:
                    score += 12
            if cluster == "editorial":
                if {"newsletter", "archives", "content", "categories", "search"} & result_tokens:
                    score += 10
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        best_color = self._select_color_match(color_results, category, query)
        best_typography = typography_results[0] if typography_results else {}
        best_landing = self._select_landing_match(
            landing_results,
            query,
            category,
        ...
        return {
            ...
            "colors": {
                "primary": self._pick_value(best_color, "Primary", "Primary (Hex)", default="#2563EB"),
                "secondary": self._pick_value(best_color, "Secondary", "Secondary (Hex)", default="#3B82F6"),
                "cta": self._pick_value(best_color, "Accent", "CTA (Hex)", default="#F97316"),
                "background": self._pick_value(best_color, "Background", "Background (Hex)", default="#F8FAFC"),
                "text": self._pick_value(best_color, "Foreground", "Text (Hex)", default="#1E293B"),
            ...
            "typography": {
                ...
                "google_fonts_url": self._extract_google_stylesheet_url(best_typography),
